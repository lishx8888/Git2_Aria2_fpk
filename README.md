# Aria2G（群晖 DSM SPK 应用）

群晖 DSM 第三方套件（SPK），提供 Aria2G 下载管理面板：支持 HTTP/FTP、BT/磁力下载，内置 BT-Tracker 与 DHT 网络，并集成 GitHub 链接加速下载。

- **内置 aria2c**：随包携带 aria2c 1.37.0（x86-64 静态编译），不依赖系统/社群套件，开箱即用
- **RPC 服务端代理**：前端不直连 aria2c，RPC 请求由 Bash CGI 转发，密钥不下发浏览器
- **桌面入口**：安装后在 DSM 主菜单出现 Aria2G 图标，点击直接打开面板
...
- 遵循 DSM 7 安全模型：守护进程以套件系统用户 `sc-Aria2G` 运行，不使用 root 权限
...
├── conf/
│   ├── privilege               # 运行身份（run-as: package → sc-Aria2G）
│   └── resource                # data-share：向导共享文件夹授权 sc-Aria2G 读写
├── scripts/                    # 生命周期脚本
...
└── package/                    # 安装后展开到 /var/packages/Aria2G/target
    ├── ui/
...
浏览器 (package/www/index.html)
   │  /webman/3rdparty/Aria2G/index.cgi/api/*
   ▼
package/ui/index.cgi (Bash CGI，DSM webman 执行)
   │  读取 @appdata/Aria2G/aria2.conf 获取端口与密钥，注入 token:secret
   │  转发到 http://127.0.0.1:<rpc_port>/jsonrpc
   ▼
aria2c (target/server/aria2c，以系统用户 sc-Aria2G 运行的守护进程)
   │  配置：/volumeX/@appdata/Aria2G/aria2.conf（由 aria2.tpl 渲染）
   ▼
...
2. 套件中心通过 `start-stop-status` 启停 aria2c，PID 记录在 `@appdata/Aria2G/aria2.pid`
3. 面板配置（加速前缀、下载目录）保存在 `@appdata/Aria2G/ui.conf`

## 路径说明

| 用途 | 路径（以 /volume1 为例） |
| --- | --- |
| 安装目录 | `/var/packages/Aria2G/target`（软链到 `/volume1/@appstore/Aria2G`） |
| 数据目录 | `/volume1/@appdata/Aria2G` |
| aria2 配置 | `@appdata/Aria2G/aria2.conf` |
| 会话 / DHT | `@appdata/Aria2G/aria2.session`、`dht.dat`、`dht6.dat` |
| aria2 日志 | `@appdata/Aria2G/aria2.log`、`service.log` |
| 面板配置 | `@appdata/Aria2G/ui.conf` |
...
chmod +x build.sh
./build.sh
# 产物：Aria2G-x86_64-1.0.2-001.spk
```
...
| 变量 | 说明 | 默认值 |
| --- | --- | --- |
| `wizard_share_name` | 下载用共享文件夹名称，不存在则自动创建，框架自动授予 `sc-Aria2G` 读写权限 | `downloads` |
| `rpc_secret` | RPC 访问密钥，仅服务端使用，不暴露给前端 | `aria2rpc_secret` |
| `rpc_port` | aria2c RPC 监听端口 | `6800` |

升级安装不弹向导，保留既有配置；如需修改密钥/端口，可 SSH 编辑 `@appdata/Aria2G/aria2.conf` 后在套件中心重启 Aria2G。
...
| `${DATA_DIR}` | `/volumeX/@appdata/Aria2G` |
| `${DOWNLOAD_DIR}` | 向导共享文件夹真实路径 |

### 运行身份与文件权限（DSM 7 安全模型）

- 守护进程与 `start-stop-status` 以系统内部用户 `sc-Aria2G` 运行（[conf/privilege](conf/privilege) 声明 `run-as: package`，用户由套件框架自动创建），不申请 root 权限，可正常安装
- 下载共享文件夹通过 [conf/resource](conf/resource) 的 `data-share` 声明，安装时由 DSM 资源服务创建/授权，`sc-Aria2G` 拥有读写 ACL
- 下载文件由 `sc-Aria2G` 创建，aria2.conf 设置 `umask=000`（文件 666/目录 777），配合共享文件夹自身的 ACL，DSM 用户经 File Station/SMB 均可正常读写
- 在面板设置里改为**其他目录**时，需先在「控制面板 → 共享文件夹 → 编辑 → 权限 → 系统内部用户」授予 `sc-Aria2G` 读写，否则该目录无法写入

## HTTP 接口（index.cgi）

经 `/webman/3rdparty/Aria2G/index.cgi` 访问，均为 GET：
...
卸载会删除 `/volumeX/@appdata/Aria2G`（任务列表、会话、配置），**不会删除**下载共享文件夹中的已下载文件（共享文件夹本身保留，如需删除请在「控制面板 → 共享文件夹」手动操作）。
...
**Q：面板打不开？**
查看 `/volumeX/@appdata/Aria2G/service.log` 与 `aria2.log`；确认套件状态为「运行中」。CGI 需读取同目录 `aria2.conf`，文件权限由 postinst 自动设置，请勿手工改为 600。
...
- [aria2](https://github.com/Aria2G/aria2) — 下载内核 aria2c（HTTP/FTP/BT/磁力，支持 RPC 与 DHT）
